...
export function exportDXF(config: TableConfig): void {
	const { length, width, legTube } = config;
	...
	entities += rect(0, 0, length, width, 'TABLE');

	// Leg rectangles at corners
	// Bottom-left
	entities += rect(0, 0, legW, legH, 'LEGS');
	// Bottom-right
	entities += rect(length - legW, 0, legW, legH, 'LEGS');
	// Top-left
	entities += rect(0, width - legH, legW, legH, 'LEGS');
	// Top-right
	entities += rect(length - legW, width - legH, legW, legH, 'LEGS');

	// Dimension text labels
	const textH = Math.max(1, Math.min(length, width) * 0.04);
	entities += text(length / 2 - textH * 2, -textH * 2, textH, `${length}"`, 'DIMS');
	entities += text(-textH * 4, width / 2, textH, `${width}"`, 'DIMS');
	entities += text(length / 2 - textH * 3, width / 2, textH, `H: ${config.height}"`, 'DIMS');
